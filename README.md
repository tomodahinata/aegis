...
Every rule is designed to produce **zero findings on a correct, production-grade RLS design** (validated against a real exemplary schema), so a finding means a real gap, not noise. The flagship owner-scoping rule (`rls/policy-not-owner-scoped` — a policy that *authenticates* the caller but never *scopes rows to the owner*) was field-tested against real production code: **1,000 public Supabase repos, ~117k RLS policies, every one of the 235 findings hand-audited to a genuine gap (precision 1.0)**. See [`research/rls-precision-study`](./research/rls-precision-study).
...
> **How this fits alongside other RLS tools.** Good tooling already exists here, and Aegis is meant to sit next to it, not above it. [`pgrls`](https://github.com/pgrls/pgrls) is an excellent *static* Postgres RLS analyzer (with a Z3 isolation prover) if your workflow is Python-first; **Supabase's own RLS Tester** is a great interactive dashboard tool for *dynamically* impersonating a user (SELECT-only, manual). Aegis's angle is the *combination for a TypeScript/Next.js team*: it reads your RLS **and** your app code in the same language, runs **statically in CI** (emitting SARIF for GitHub code-scanning) across `SELECT`/`INSERT`/`UPDATE`/`DELETE` policies, and — with `aegis probe` — can turn a static "maybe" into an **actually-reproduced HTTP exchange** (SAST↔DAST correlation). Different tools, overlapping goal; use whatever fits your stack.
